...
const aboutText = [
    ...
    'PARCOURS',
    '--------',
    ...profile.career.flatMap((job) => [`${job.period}  ${job.role} — ${job.company} (${job.place})`, `    ${job.summary}`]),
    '',
    'FORMATION',
    '---------',
    ...profile.education.map((item) => `${item.period.padEnd(13)} ${item.title} — ${item.school}`),
    '',
